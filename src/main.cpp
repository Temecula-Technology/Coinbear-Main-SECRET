...
#if defined(NDEBUG)
# error "Coinbear cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd11b79dfac4fc56fdb79f045c92997e08219dcfdb9bd97936e4eb376fe7f2a5c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coinbear: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coinbear Signed Message:\n";
...
{
    // Coinbear: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Coinbear: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Coinbear: 3 days
static const int64 nTargetSpacing = 2 * 60; // Coinbear: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coinbear: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coinbear: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd8053c0db4283085c9b8a616b37eeaeed8710035fee62d4c0e04e532c38b211c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 05/16/2021 Nolan and his CoinBear COVID-19 PandemicCases 162590514 People ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e523b529358a8548a3e3176b8f2b72078685b338a85e4d1aaa827f76df0d87c47023836d8134f9664000358ae5d8e6011fb51a11e73c0be427cc4f23c415840e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1621191881;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085165537;

        if (fTestNet)
        {
            block.nTime    = 1621191873;
            block.nNonce   = 385580137;
        }


		

	//Genesis Block Creation Stuff
	...
	//END GENESIS Creation
        ...
        assert(block.hashMerkleRoot == uint256("0x293756e356a561f7567e3400d7319677037667520fc2138f1a791944292a3458"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc9, 0xb3, 0xdl }; // Coinbear: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoinbearMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoinbearMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoinbearMiner(CWallet *pwallet)
{
    printf("CoinbearMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coinbear-miner");
    ...
    try { loop {
        ...
        printf("Running CoinbearMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoinbearMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoinbearMiner, pwallet));
}
